...
/* drag and drop */
	...
	saveAjax: function(fileName, startTemplateUrl, page_title, page_description,callback, saveUrl)
	{
		var data = {};
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(url, null,null,null, null, saveUrl).done(function (data, text) {
			/*
	...
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( e ) {
			...
			return Vvveb.Builder.saveAjax(data.file, data.startTemplateUrl,data.page_title,data.page_desc, function (data) {
...
Vvveb.FileManager = {
	...
	deletePage: function(element, e) {
		...
		if (confirm(`Are you sure you want to delete "${page.file}"template?`)) {
			$.ajax({
				type: "POST",
				url: "save.php?action=delete",//set your server side save script url
				data: {file:page.url},
	...
	renamePage: function(element, e, duplicate = false) {
		let page = element[0].dataset;
		let newfile = prompt(`Enter new file name for "${page.url}"`, page.url);
		let _self = this;
		if (newfile) {
			$.ajax({
				type: "POST",
				url: "save.php?action=rename",//set your server side save script url
				data: {file:page.url, newfile:newfile, duplicate},
				success: function (data, text) {
					let bg = "bg-success";
					if (data.success) {		
						$(".save-btn").attr("disabled", "true");
